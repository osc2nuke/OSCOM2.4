<?php
	...
	*/
		...
		if (tep_not_null($_POST['firstname']) && tep_not_null($_POST['lastname']) && tep_not_null($_POST['street_address'])) {
			...
			if (ACCOUNT_GENDER == 'true') {
				if ( ($gender != 'm') && ($gender != 'f') ) {
					...
					$messageStack->add('checkout_address', $osC_Language->get('ENTRY_GENDER_ERROR'));
				}
			}
			...
			if (strlen($firstname) < ENTRY_FIRST_NAME_MIN_LENGTH) {
				...
				$messageStack->add('checkout_address', $osC_Language->get('ENTRY_FIRST_NAME_ERROR'));
			}
			
			if (strlen($lastname) < ENTRY_LAST_NAME_MIN_LENGTH) {
				$error = true;
				
				$messageStack->add('checkout_address', $osC_Language->get('ENTRY_LAST_NAME_ERROR'));
			}
			
			if (strlen($street_address) < ENTRY_STREET_ADDRESS_MIN_LENGTH) {
				$error = true;
				
				$messageStack->add('checkout_address', $osC_Language->get('ENTRY_STREET_ADDRESS_ERROR'));
			}
			
			if (strlen($postcode) < ENTRY_POSTCODE_MIN_LENGTH) {
				$error = true;
				
				$messageStack->add('checkout_address', $osC_Language->get('ENTRY_POST_CODE_ERROR'));
			}
			
			if (strlen($city) < ENTRY_CITY_MIN_LENGTH) {
				$error = true;
				
				$messageStack->add('checkout_address', $osC_Language->get('ENTRY_CITY_ERROR'));
			}
			...
			if (ACCOUNT_STATE == 'true') {
				...
				if ($entry_state_has_zones == true) {
					...
					if (count($Qzone->fetchAll()) === 1) {
						...
						$messageStack->add('checkout_address', $osC_Language->get('ENTRY_STATE_ERROR_SELECT'));
					}
					} else {
					if (strlen($state) < ENTRY_STATE_MIN_LENGTH) {
						$error = true;
						
						$messageStack->add('checkout_address', $osC_Language->get('ENTRY_STATE_ERROR'));
					}
				}
			}
			...
			if ( (is_numeric($country) == false) || ($country < 1) ) {
				...
				$messageStack->add('checkout_address', $osC_Language->get('ENTRY_COUNTRY_ERROR'));
			}
	...
	$breadcrumb->add($osC_Language->get('CHECKOUT_SHIPPING_ADDRESS_NAVBAR_TITLE_1'), OSCOM::link('checkout_shipping.php', '', 'SSL'));
	$breadcrumb->add($osC_Language->get('CHECKOUT_SHIPPING_ADDRESS_NAVBAR_TITLE_2'), OSCOM::link('checkout_shipping_address.php', '', 'SSL'));
...
<div class="page-header">
	<h1><?php echo $osC_Language->get('CHECKOUT_SHIPPING_ADDRESS_HEADING_TITLE'); ?></h1>
</div>
...
<div class="contentContainer">
	...
	<?php
		...
		<div class="page-header">
			<h4><?php echo $osC_Language->get('CHECKOUT_SHIPPING_ADDRESS_TABLE_HEADING_SHIPPING_ADDRESS'); ?></h4>
		</div>
		
		<div class="contentText row">
			<div class="col-sm-8">
				<div class="alert alert-warning">
					<?php echo $osC_Language->get('CHECKOUT_SHIPPING_ADDRESS_TEXT_SELECTED_SHIPPING_DESTINATION'); ?>
				</div>
			</div>
			<div class="col-sm-4">
				<div class="panel panel-primary">
					<div class="panel-heading"><?php echo $osC_Language->get('CHECKOUT_SHIPPING_ADDRESS_TITLE_SHIPPING_ADDRESS'); ?></div>
		...
		<?php
			...
			<div class="page-header">
				<h4><?php echo $osC_Language->get('CHECKOUT_SHIPPING_ADDRESS_TABLE_HEADING_ADDRESS_BOOK_ENTRIES'); ?></h4>
			</div>
			
			<div class="contentText">
				<div class="alert alert-warning">
					<div class="pull-right">
						<?php echo '<strong>' . $osC_Language->get('CHECKOUT_SHIPPING_ADDRESS_TITLE_PLEASE_SELECT') . '</strong>'; ?>
					</div>
					
					<?php echo $osC_Language->get('CHECKOUT_SHIPPING_ADDRESS_TEXT_SELECT_OTHER_SHIPPING_DESTINATION'); ?>
				</div>
			</div>
	...
	<div class="page-header">
		<h4><?php echo $osC_Language->get('CHECKOUT_SHIPPING_ADDRESS_TABLE_HEADING_NEW_SHIPPING_ADDRESS'); ?></h4>
	</div>
	
	<div class="contentText">
		<div class="alert alert-info">
			<?php echo $osC_Language->get('CHECKOUT_SHIPPING_ADDRESS_TEXT_CREATE_NEW_SHIPPING_ADDRESS'); ?>
		</div>
	</div>
...
<div class="contentText">
    <div><?php echo HTML::hiddenField('action', 'submit') . HTML::button($osC_Language->get('IMAGE_BUTTON_CONTINUE'), 'glyphicon glyphicon-chevron-right', null, 'primary', null, 'btn-success btn-block'); ?></div>
</div>
...
<?php
	...
	<div>
		<?php echo HTML::button($osC_Language->get('IMAGE_BUTTON_BACK'), 'glyphicon glyphicon-chevron-left', OSCOM::link('checkout_shipping_address.php', '', 'SSL')); ?>
	</div>
