	...
	*/
	...
	class osC_Language {
		...
		function osC_Language() {
			...
			$Qlanguages = $OSCOM_Db->prepare('select * from :table_osc_languages order by sort_order, name');
		...
		function load($key, $language_code = null) {
			...
			$Qdef = $OSCOM_Db->prepare('select * from :table_osc_languages_definitions where languages_id = :languages_id and content_group = :content_group');
