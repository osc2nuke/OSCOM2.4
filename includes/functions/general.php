<?php
	...
	*/
	...
	function tep_get_products_name($product_id, $language_id = null) {
		...
		if (!isset($language_id)) $language_id = $osC_Language->getID();
	...
	function tep_check_stock($products_id, $products_quantity) {
		...
		if ($stock_left < 0) {
			$out_of_stock = '<span class="markProductOutOfStock">' . $osC_Language->get('STOCK_MARK_PRODUCT_OUT_OF_STOCK') . '</span>';
		}
	...
	function tep_get_tax_description($class_id, $country_id, $zone_id) {
		...
		if (!isset($tax_rates[$class_id][$country_id][$zone_id]['description'])) {
			...
			if ($Qtax->fetch() !== false) {
				...
				$tax_rates[$class_id][$country_id][$zone_id]['description'] = $osC_Language->get('TEXT_UNKNOWN_TAX_RATE');
			}
		}
	...
	function tep_create_sort_heading($sortby, $colnum, $heading) {
		global $PHP_SELF, $osC_Language;
		
		$sort_prefix = '';
		$sort_suffix = '';
		
		if ($sortby) {
			$sort_prefix = '<a href="' . OSCOM::link($PHP_SELF, tep_get_all_get_params(array('page', 'info', 'sort')) . 'page=1&sort=' . $colnum . ($sortby == $colnum . 'a' ? 'd' : 'a')) . '" title="' . tep_output_string($osC_Language->get('TEXT_SORT_PRODUCTS') . ($sortby == $colnum . 'd' || substr($sortby, 0, 1) != $colnum ? $osC_Language->get('TEXT_ASCENDINGLY') : $osC_Language->get('TEXT_DESCENDINGLY')) . $osC_Language->get('TEXT_BY') . $heading) . '" class="productListing-heading">' ;
	...
	function tep_customer_greeting() {
		...
		if (isset($_SESSION['customer_first_name']) && isset($_SESSION['customer_id'])) {
			$greeting_string = sprintf($osC_Language->get('TEXT_GREETING_PERSONAL'), tep_output_string_protected($_SESSION['customer_first_name']), OSCOM::link('products_new.php'));
			} else {
			$greeting_string = sprintf($osC_Language->get('TEXT_GREETING_GUEST'), OSCOM::link('login.php', '', 'SSL'), OSCOM::link('create_account.php', '', 'SSL'));
		}
