	...
	*/
	...
	class cm_pi_reviews {
		...
		function cm_pi_reviews() {
			...
			$this->title = $osC_Language->get('MODULE_CONTENT_PRODUCT_INFO_REVIEWS_TITLE');
			$this->description = $osC_Language->get('MODULE_CONTENT_PRODUCT_INFO_REVIEWS_DESCRIPTION');
		...
		function execute() {
			...
			if ($Qreviews->fetch() !== false) {
				...
				do {
					...
					'  <footer>' . sprintf($osC_Language->get('MODULE_CONTENT_PRODUCT_INFO_REVIEWS_TEXT_RATED'), HTML::stars($Qreviews->valueInt('reviews_rating')), $Qreviews->valueProtected('customers_name'), $Qreviews->valueProtected('customers_name')) . '</footer>' .
					'</blockquote>';
