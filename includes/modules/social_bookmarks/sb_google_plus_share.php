	...
	*/
	...
	class sb_google_plus_share {
		...
		function sb_google_plus_share() {
			global $osC_Language;
			$this->title = $osC_Language->get('MODULE_SOCIAL_BOOKMARKS_GOOGLE_PLUS_SHARE_TITLE');
			$this->public_title = $osC_Language->get('MODULE_SOCIAL_BOOKMARKS_GOOGLE_PLUS_SHARE_PUBLIC_TITLE');
			$this->description = $osC_Language->get('MODULE_SOCIAL_BOOKMARKS_GOOGLE_PLUS_SHARE_DESCRIPTION');
		...
		function getOutput() {
			...
			foreach ($lng->catalog_languages as $lkey => $lvalue) {
				if ($lvalue['id'] == $osC_Language->getID()) {
