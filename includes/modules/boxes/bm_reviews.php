<?php
	...
	*/
	...
	class bm_reviews {
		...
		function bm_reviews() {
			global $osC_Language;
			$this->title = $osC_Language->get('MODULE_BOXES_REVIEWS_TITLE');
			$this->description = $osC_Language->get('MODULE_BOXES_REVIEWS_DESCRIPTION');
		...
		function execute() {
			global $currencies, $osC_Language, $oscTemplate;
			...
			if (count($result) > 0) {
				...
				$reviews_box_contents = '<span class="glyphicon glyphicon-thumbs-up"></span> <a href="' . OSCOM::link('product_reviews_write.php', 'products_id=' . (int)$_GET['products_id']) . '">' . $osC_Language->get('MODULE_BOXES_REVIEWS_BOX_WRITE_REVIEW') .'</a>';
				} else {
				// display 'no reviews' box
				$reviews_box_contents = '<p>' . $osC_Language->get('MODULE_BOXES_REVIEWS_BOX_NO_REVIEWS') . '</p>';
			}
