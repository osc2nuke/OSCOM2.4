	...
	*/
	...
	class bm_card_acceptance {
		...
		function bm_card_acceptance() {
			global $osC_Language;
			$this->title = $osC_Language->get('MODULE_BOXES_CARD_ACCEPTANCE_TITLE');
			$this->description = $osC_Language->get('MODULE_BOXES_CARD_ACCEPTANCE_DESCRIPTION');
		...
		function execute() {
			global $PHP_SELF, $osC_Language, $oscTemplate;
	...
	function bm_card_acceptance_edit_logos($values, $key) {
		...
		$output = '<h3>' . $osC_Language->get('MODULE_BOXES_CARD_ACCEPTANCE_SHOWN_CARDS') . '</h3>' .
		...
		$output .= '<h3>' . $osC_Language->get('MODULE_BOXES_CARD_ACCEPTANCE_NEW_CARDS') . '</h3><ul id="new_ca_logos" style="list-style-type: none; margin: 0; padding: 5px; margin-bottom: 10px;">';
		...
		$drag_here_li = '<li id="caLogoEmpty" style="background-color: #fcf8e3; border: 1px #faedd0 solid; color: #a67d57; padding: 5px;">' . addslashes($osC_Language->get('MODULE_BOXES_CARD_ACCEPTANCE_DRAG_HERE')) . '</li>';
