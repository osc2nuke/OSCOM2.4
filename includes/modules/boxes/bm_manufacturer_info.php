	...
	*/
	...
	class bm_manufacturer_info {
		...
		function bm_manufacturer_info() {
			global $osC_Language;
			$this->title = $osC_Language->get('MODULE_BOXES_MANUFACTURER_INFO_TITLE');
			$this->description = $osC_Language->get('MODULE_BOXES_MANUFACTURER_INFO_DESCRIPTION');
			...
			if (isset($_GET['products_id'])) {
				...
				if ($Qmanufacturer->fetch() !== false) {
					...
					if (!empty($Qmanufacturer->value('manufacturers_url'))) {
						$manufacturer_info_string .= '<div class="text-center"><a href="' . OSCOM::link('redirect.php', 'action=manufacturer&manufacturers_id=' . $Qmanufacturer->valueInt('manufacturers_id')) . '" target="_blank">' . sprintf($osC_Language->get('MODULE_BOXES_MANUFACTURER_INFO_BOX_HOMEPAGE'), $Qmanufacturer->value('manufacturers_name')) . '</a></div>';
					}
