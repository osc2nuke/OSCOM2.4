	...
	*/
	...
	class bm_product_notifications {
		...
		function bm_product_notifications() {
			global $osC_Language;
			$this->title = $osC_Language->get('MODULE_BOXES_PRODUCT_NOTIFICATIONS_TITLE');
			$this->description = $osC_Language->get('MODULE_BOXES_PRODUCT_NOTIFICATIONS_DESCRIPTION');
		...
		function execute() {
			global $PHP_SELF, $request_type, $osC_Language, $oscTemplate;
			...
			if (isset($_GET['products_id'])) {
				...
				if ($notification_exists == true) {
					$notif_contents = '<a href="' . OSCOM::link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=notify_remove', $request_type) . '"><span class="glyphicon glyphicon-remove"></span> ' . sprintf($osC_Language->get('MODULE_BOXES_PRODUCT_NOTIFICATIONS_BOX_NOTIFY_REMOVE'), tep_get_products_name($_GET['products_id'])) .'</a>';
					} else {
					$notif_contents = '<a href="' . OSCOM::link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=notify', $request_type) . '"><span class="glyphicon glyphicon-envelope"></span> ' . sprintf($osC_Language->get('MODULE_BOXES_PRODUCT_NOTIFICATIONS_BOX_NOTIFY'), tep_get_products_name($_GET['products_id'])) .'</a>';
				}
