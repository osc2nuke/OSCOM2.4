<?php
	...
	*/
	...
	class zones {
		var $code, $title, $description, $enabled, $num_zones, $osC_Language;
		
		// class constructor
		function zones() {
			$this->code = 'zones';
			$this->title = $osC_Language->get('MODULE_SHIPPING_ZONES_TEXT_TITLE');
			$this->description = $osC_Language->get('MODULE_SHIPPING_ZONES_TEXT_DESCRIPTION');
		...
		function quote($method = '') {
			global $order, $shipping_weight, $shipping_num_boxes, $osC_Language;
			...
			if ($dest_zone == 0) {
				...
				for ($i=0; $i<$size; $i+=2) {
					if ($shipping_weight <= $zones_table[$i]) {
						...
						$shipping_method = $osC_Language->get('MODULE_SHIPPING_ZONES_TEXT_WAY') . ' ' . $dest_country . ' : ' . $shipping_weight . ' ' . $osC_Language->get('MODULE_SHIPPING_ZONES_TEXT_UNITS');
						break;
					}
				}
				...
				if ($shipping == -1) {
					$shipping_cost = 0;
					$shipping_method = $osC_Language->get('MODULE_SHIPPING_ZONES_UNDEFINED_RATE');
					} else {
			...
			'module' => $osC_Language->get('MODULE_SHIPPING_ZONES_TEXT_TITLE'),
			...
			if ($error == true) $this->quotes['error'] = $osC_Language->get('MODULE_SHIPPING_ZONES_INVALID_ZONE');
