	...
	*/
	...
	class ht_category_title {
		...
		function ht_category_title() {
			global $osC_Language;
			$this->title = $osC_Language->get('MODULE_HEADER_TAGS_CATEGORY_TITLE_TITLE');
			$this->description = $osC_Language->get('MODULE_HEADER_TAGS_CATEGORY_TITLE_DESCRIPTION');
		...
		function execute() {
			global $PHP_SELF, $current_category_id, $osC_Language, $oscTemplate;
			
			$OSCOM_Db = Registry::get('Db');
			
			if (basename($PHP_SELF) == 'index.php') {
				if ($current_category_id > 0) {
					$Qcategory = $OSCOM_Db->get('categories_description', 'categories_name', ['categories_id' => $current_category_id, 'language_id' => $osC_Language->getID()]);
