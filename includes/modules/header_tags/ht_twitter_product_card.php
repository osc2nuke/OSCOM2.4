	...
	*/
	...
	class ht_twitter_product_card {
		...
		function ht_twitter_product_card() {
			global $osC_Language;
			$this->title = $osC_Language->get('MODULE_HEADER_TAGS_TWITTER_PRODUCT_CARD_TITLE');
			$this->description = $osC_Language->get('MODULE_HEADER_TAGS_TWITTER_PRODUCT_CARD_DESCRIPTION');
		...
		function execute() {
			global $PHP_SELF, $currencies, $osC_Language, $oscTemplate;
			...
			if ( ($PHP_SELF == 'product_info.php') && isset($_GET['products_id']) ) {
				...
				if ($Qproduct->fetch() !== false) {
					...
					if ( $Qproduct->value('products_date_available') > date('Y-m-d H:i:s') ) {
						$data['data2'] = $osC_Language->get('MODULE_HEADER_TAGS_TWITTER_PRODUCT_CARD_TEXT_PRE_ORDER');
						$data['label2'] = tep_date_short($Qproduct->value('products_date_available'));
						} elseif ( $Qproduct->valueInt('products_quantity') > 0 ) {
						$data['data2'] = $osC_Language->get('MODULE_HEADER_TAGS_TWITTER_PRODUCT_CARD_TEXT_IN_STOCK');
						$data['label2'] = $osC_Language->get('MODULE_HEADER_TAGS_TWITTER_PRODUCT_CARD_TEXT_BUY_NOW');
						} else {
						$data['data2'] = $osC_Language->get('MODULE_HEADER_TAGS_TWITTER_PRODUCT_CARD_TEXT_OUT_OF_STOCK');
						$data['label2'] = $osC_Language->get('MODULE_HEADER_TAGS_TWITTER_PRODUCT_CARD_TEXT_CONTACT_US');
					}
