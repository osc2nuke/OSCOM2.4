...
*/
  ...
  class osC_Services_debug {
    function start() {
      ...
      if ((SERVICE_DEBUG_CHECK_CONFIGURATION == '1') && file_exists(dirname($_SERVER['SCRIPT_FILENAME']) . '/includes/configure_admin.php') && is_writeable(dirname($_SERVER['SCRIPT_FILENAME']) . '/includes/configure_admin.php')) {
        $osC_MessageStack->add('debug', sprintf($osC_Language->get('warning_config_file_writeable'), dirname($_SERVER['SCRIPT_FILENAME']) . '/includes/configure_admin.php') . ' [' . __CLASS__ . ']', 'warning');
      }
