<?php
	...
	*/
	...
	<?php
		...
		$catname = $osC_Language->get('INDEX_HEADING_TITLE');
		if (isset($_GET['manufacturers_id']) && !empty($_GET['manufacturers_id'])) {
			...
			$Qtitle->bindInt(':language_id', $osC_Language->getID());
			$Qtitle->execute();
		...
		<?php
			...
			if (PRODUCT_LIST_FILTER > 0) {
				if (isset($_GET['manufacturers_id']) && !empty($_GET['manufacturers_id'])) {
					...
					$Qfilter->bindInt(':language_id', $osC_Language->getID());
				...
				if ($Qfilter->getPageSetTotalRows() > 1) {
					echo '<div>' . HTML::form('filter', OSCOM::link('index.php', '', $request_type, false), 'get', null, ['session_id' => true]) . '<p align="right">' . $osC_Language->get('INDEX_TEXT_SHOW') . '&nbsp;';
					if (isset($_GET['manufacturers_id']) && !empty($_GET['manufacturers_id'])) {
						echo HTML::hiddenField('manufacturers_id', $_GET['manufacturers_id']);
						$options = array(array('id' => '', 'text' => $osC_Language->get('INDEX_TEXT_ALL_CATEGORIES')));
						} else {
						echo HTML::hiddenField('cPath', $cPath);
						$options = array(array('id' => '', 'text' => $osC_Language->get('INDEX_TEXT_ALL_MANUFACTURERS')));
					}
	...
	<div class="page-header">
		<h1><?php echo sprintf($osC_Language->get('INDEX_HEADING_TITLE'), STORE_NAME); ?></h1>
	</div>
	...
	<div class="contentContainer">
		...
		<?php
			if (tep_not_null($osC_Language->get('INDEX_TEXT_MAIN'))) {
			?>
			
			<div class="contentText">
				<?php echo $osC_Language->get('INDEX_TEXT_MAIN'); ?>
			</div>
