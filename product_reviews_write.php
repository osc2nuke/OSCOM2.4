	...
	*/
	...
	if (isset($_GET['action']) && ($_GET['action'] == 'process') && isset($_POST['formid']) && ($_POST['formid'] == $_SESSION['sessiontoken'])) {
		...
		if (strlen($review) < REVIEW_TEXT_MIN_LENGTH) {
			...
			$messageStack->add('review', $osC_Language->get('JS_REVIEW_TEXT'));
		}
		
		if (($rating < 1) || ($rating > 5)) {
			$error = true;
			
			$messageStack->add('review', $osC_Language->get('JS_REVIEW_RATING'));
		}
		...
		if ($error == false) {
			...
			$OSCOM_Db->save('reviews_description', ['reviews_id' => $insert_id, 'languages_id' => $osC_Language->getID(), 'reviews_text' => $review]);
			
			$messageStack->add_session('product_reviews', $osC_Language->get('PRODUCT_REVIEWS_WRITE_TEXT_REVIEW_RECEIVED'), 'success');
			OSCOM::redirect('product_reviews.php', tep_get_all_get_params(array('action')));
		}
	}
	...
	$breadcrumb->add($osC_Language->get('PRODUCT_REVIEWS_WRITE_NAVBAR_TITLE'), OSCOM::link('product_reviews.php', tep_get_all_get_params()));
...
<div class="contentContainer">
	...
	<?php
		...
		<div class="col-sm-4 text-center pull-right">
			...
			<p><?php echo HTML::button($osC_Language->get('IMAGE_BUTTON_IN_CART'), 'glyphicon glyphicon-shopping-cart', OSCOM::link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=buy_now'), null, null, 'btn-success btn-block'); ?></p>
		</div>
	...
	<div class="contentText">
		<div class="row">
			<p class="col-sm-3 text-right-not-xs"><strong><?php echo $osC_Language->get('PRODUCT_REVIEWS_WRITE_SUB_TITLE_FROM'); ?></strong></p>
			<p class="col-sm-9"><?php echo HTML::sanitize($Qcustomer->value('customers_firstname') . ' ' . $Qcustomer->value('customers_lastname')); ?></p>
		</div>
		<div class="form-group has-feedback">
			<label for="inputReview" class="control-label col-sm-3"><?php echo $osC_Language->get('PRODUCT_REVIEWS_WRITE_SUB_TITLE_REVIEW'); ?></label>
			<div class="col-sm-9">
				<?php
					echo HTML::textareaField('review', 60, 15, NULL, 'minlength="' . REVIEW_TEXT_MIN_LENGTH . '" required aria-required="true" id="inputReview" placeholder="' . $osC_Language->get('ENTRY_REVIEW_TEXT') . '"');
					echo $osC_Language->get('FORM_REQUIRED_INPUT');
				?>
			</div>
		</div>
		<div class="form-group">
			<label class="control-label col-sm-3"><?php echo $osC_Language->get('PRODUCT_REVIEWS_WRITE_SUB_TITLE_RATING'); ?></label>
			<div class="col-sm-9">
				<div class="radio">
					<label>
						<?php echo HTML::radioField('rating', '5') . HTML::stars(5, false) . ' ' . $osC_Language->get('PRODUCT_REVIEWS_WRITE_TEXT_GOOD'); ?>
					</label>
				</div>
				<div class="radio">
					<label>
				...
				<div class="radio">
					<label>
						<?php echo HTML::radioField('rating', '1', null, 'required aria-required="true"') . HTML::stars(1, false) . ' ' . $osC_Language->get('PRODUCT_REVIEWS_WRITE_TEXT_BAD'); ?>
					</label>
				</div>
			</div>
		</div>
	...
	<div class="row">
		...
		<div class="col-xs-6"><?php echo HTML::button($osC_Language->get('IMAGE_BUTTON_BACK'), 'glyphicon glyphicon-chevron-left', OSCOM::link('product_reviews.php', tep_get_all_get_params(array('reviews_id', 'action')))); ?></div>
	</div>
