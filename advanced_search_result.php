<?php
	...
	*/
	...
	(isset($_GET['pto']) && !is_numeric($_GET['pto'])) ) {
		...
		$messageStack->add_session('search', $osC_Language->get('ADVANCED_SEARCH_ERROR_AT_LEAST_ONE_INPUT'));
		} else {
		$dfrom = '';
		$dto = '';
		$pfrom = '';
		...
		if (tep_not_null($dfrom)) {
			if (!tep_checkdate($dfrom, DOB_FORMAT_STRING, $dfrom_array)) {
				...
				$messageStack->add_session('search', $osC_Language->get('ADVANCED_SEARCH_ERROR_INVALID_FROM_DATE'));
			}
		}
		...
		if (tep_not_null($dto)) {
			if (!tep_checkdate($dto, DOB_FORMAT_STRING, $dto_array)) {
				...
				$messageStack->add_session('search', $osC_Language->get('ADVANCED_SEARCH_ERROR_INVALID_TO_DATE'));
			}
		}
		...
		if (($date_check_error == false) && tep_not_null($dfrom) && tep_not_null($dto)) {
			if (mktime(0, 0, 0, $dfrom_array[1], $dfrom_array[2], $dfrom_array[0]) > mktime(0, 0, 0, $dto_array[1], $dto_array[2], $dto_array[0])) {
				...
				$messageStack->add_session('search', $osC_Language->get('ADVANCED_SEARCH_ERROR_TO_DATE_LESS_THAN_FROM_DATE'));
			}
		}
		...
		if (tep_not_null($pfrom)) {
			if (!settype($pfrom, 'double')) {
				...
				$messageStack->add_session('search', $osC_Language->get('ADVANCED_SEARCH_ERROR_PRICE_FROM_MUST_BE_NUM'));
			}
		}
		...
		if (tep_not_null($pto)) {
			if (!settype($pto, 'double')) {
				...
				$messageStack->add_session('search', $osC_Language->get('ADVANCED_SEARCH_ERROR_PRICE_TO_MUST_BE_NUM'));
			}
		}
		...
		if (($price_check_error == false) && is_float($pfrom) && is_float($pto)) {
			if ($pfrom >= $pto) {
				...
				$messageStack->add_session('search', $osC_Language->get('ADVANCED_SEARCH_ERROR_PRICE_TO_LESS_THAN_PRICE_FROM'));
			}
		}
		...
		if (tep_not_null($keywords)) {
			...
			if (empty($search_keywords)) {
				...
				$messageStack->add_session('search', $osC_Language->get('ADVANCED_SEARCH_ERROR_INVALID_KEYWORDS'));
			}
		}
	}
	...
	$breadcrumb->add($osC_Language->get('ADVANCED_SEARCH_NAVBAR_TITLE_1'), OSCOM::link('advanced_search.php'));
	$breadcrumb->add($osC_Language->get('ADVANCED_SEARCH_NAVBAR_TITLE_2'), OSCOM::link('advanced_search_result.php', tep_get_all_get_params(), 'NONSSL', true, false));
	
	require('includes/template_top.php');
?>

<div class="page-header">
	<h1><?php echo $osC_Language->get('ADVANCED_SEARCH_HEADING_TITLE_2'); ?></h1>
</div>
...
<div class="contentContainer">
	...
	<div>
		<?php echo HTML::button($osC_Language->get('IMAGE_BUTTON_BACK'), 'glyphicon glyphicon-chevron-left', OSCOM::link('advanced_search.php', tep_get_all_get_params(array('sort', 'page')), 'NONSSL', true, false)); ?>
	</div>
</div>
